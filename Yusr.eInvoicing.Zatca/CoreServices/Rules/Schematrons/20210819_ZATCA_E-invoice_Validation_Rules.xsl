...
<!--                  </xsl:if>-->
<!--               </xsl:if>-->
               <xsl:if test="boolean(//cbc:InvoiceTypeCode[
   string-length(@name) = 9
   and ((substring(@name, 1, 2) = '01') or (substring(@name, 1, 2) = '02') )
   and substring(@name, 9, 1) = '1'
])">
                  <xsl:if test="not(exists(cac:ContractDocumentReference/cbc:ID) and (normalize-space(cac:ContractDocumentReference/cbc:ID) != ''))">
                     <svrl:failed-assert xmlns:svrl="http://purl.oclc.org/dsdl/svrl" location="{schxslt:location(.)}"
                                         flag="error" id="BR-KSA-92">
                        <xsl:attribute name="test">not(exists(cac:ContractDocumentReference/cbc:ID))</xsl:attribute>
                        <svrl:text>[BR-KSA-92]-If it's a B2G invoice transaction type (KSA 2, position 9 = 01), then the
               ...
               <xsl:if test="not( exists(cac:AccountingSupplierParty/cac:Party/cac:PartyIdentification/cbc:ID) and string-length(cac:AccountingSupplierParty/cac:Party/cac:PartyIdentification/cbc:ID/normalize-space(@schemeID)) &gt; 2 and contains('CRN MOM MLS SAG OTH 700', normalize-space(cac:AccountingSupplierParty/cac:Party/cac:PartyIdentification/cbc:ID/@schemeID)))">
                  <svrl:failed-assert xmlns:svrl="http://purl.oclc.org/dsdl/svrl" location="{schxslt:location(.)}" flag="warning" id="BR-KSA-08">
                     <xsl:attribute name="test">not( string-length(normalize-space( ./@schemeID)) &gt; 2 and contains('CRN MOM MLS SAG OTH 700', ./@schemeID)  and (matches((.), '^[a-zA-Z0-9]*$')) )</xsl:attribute>
      ...
      <xsl:choose>
         ...
         <xsl:otherwise>
            <schxslt:rule pattern="d7e29">
               ...
               <xsl:if test="((. castable as xs:date ) and normalize-space(.)!='') and xs:date(.) > current-date()">
  ...
  (
    (
      (string-length(@name) = 7 and
        (substring(@name,1,2) = '01' or substring(@name,1,2) = '02') and
        (substring(@name,3,1) = '0' or substring(@name,3,1) = '1') and
        (substring(@name,4,1) = '0' or substring(@name,4,1) = '1') and
        (substring(@name,5,1) = '0' or substring(@name,5,1) = '1') and
        (substring(@name,6,1) = '0' or substring(@name,6,1) = '1') and
        (substring(@name,7,1) = '0' or substring(@name,7,1) = '1')
      ) or
      (string-length(@name) = 9 and
        ...
        (substring(@name,5,1) = '0' or substring(@name,5,1) = '1') and
        (substring(@name,6,1) = '0' or substring(@name,6,1) = '1') and
        (substring(@name,7,1) = '0' or substring(@name,7,1) = '1') and
        (substring(@name,8,1) = '0' or substring(@name,8,1) = '1') and
        (substring(@name,9,1) = '0' or substring(@name,9,1) = '1')
      )
      or
      (string-length(@name) = 8 and
        (substring(@name,1,2) = '01' or substring(@name,1,2) = '02') and
        (substring(@name,3,1) = '0' or substring(@name,3,1) = '1') and
        (substring(@name,4,1) = '0' or substring(@name,4,1) = '1') and
        (substring(@name,5,1) = '0' or substring(@name,5,1) = '1') and
        (substring(@name,6,1) = '0' or substring(@name,6,1) = '1') and
        (substring(@name,7,1) = '0' or substring(@name,7,1) = '1') and
        (substring(@name,8,1) = '0' or substring(@name,8,1) = '1')
      )
    )
  )
)">
               ...
               <xsl:if test="not((((string-length(@name)) &gt;= 7) and ((string-length(@name)) &lt;= 9) ))">
                  <svrl:failed-assert xmlns:svrl="http://purl.oclc.org/dsdl/svrl" location="{schxslt:location(.)}" flag="warning" id="BR-KSA-F-06-C40">
                     <xsl:attribute name="test"></xsl:attribute>
                     <svrl:text>[BR-KSA-F-06-C40]-Field character limits for Invoice transaction code (KSA-2) have not been met. The minimum limit is 7 characters and the maximum limit is 9 characters.</svrl:text>
                     <svrl:message-code>BR-KSA-F-06-C40</svrl:message-code>
                     <svrl:message-category>KSA - business rules (BR-KSA)</svrl:message-category>
                  </svrl:failed-assert>
               </xsl:if>
            </schxslt:rule>
...
<!--      </xsl:choose>-->
<!--   </xsl:template>-->
   <!--  new template sprint-13 -->
   <xsl:template match="cac:AccountingSupplierParty | cac:AccountingCustomerParty" priority="50" mode="d7e29">
      <xsl:param name="schxslt:patterns-matched" as="xs:string*"/>
      <schxslt:rule pattern="d7e29">
         <svrl:fired-rule xmlns:svrl="http://purl.oclc.org/dsdl/svrl">
         ...
         <xsl:if test="(exists(cac:Party/cac:PartyIdentification/cbc:ID) and normalize-space(cac:Party/cac:PartyIdentification/cbc:ID/@schemeID) = 'CRN') and not(
        string-length(normalize-space(cac:Party/cac:PartyIdentification/cbc:ID)) = 10 and
        not(translate((cac:Party/cac:PartyIdentification/cbc:ID), '0123456789', '')))">
            <svrl:failed-assert xmlns:svrl="http://purl.oclc.org/dsdl/svrl"
                                location="{schxslt:location(.)}"
                                flag="warning"
                                id="BR-KSA-F-08">
               <xsl:attribute name="test">
                  (exists(cac:Party/cac:PartyIdentification/cbc:ID) and normalize-space(cac:Party/cac:PartyIdentification/cbc:ID/@schemeID) = 'CRN') and not(
                  string-length(normalize-space(cac:Party/cac:PartyIdentification/cbc:ID)) = 10 and
                  not(translate(normalize-space(cac:Party/cac:PartyIdentification/cbc:ID), '0123456789', '')))
               </xsl:attribute>
               <svrl:text>[BR-KSA-F-08] - The value provided in Other Seller ID (BT-29) or Other Buyer ID (BT-46) for the scheme ID 'CRN' appears to be
                  incorrect. Please recheck the value to avoid non-compliance.
               </svrl:text>
               <svrl:message-code>BR-KSA-F-08</svrl:message-code>
               <svrl:message-category>KSA - custom rules</svrl:message-category>
            </svrl:failed-assert>
         </xsl:if>
         <xsl:if test="(exists(cac:Party/cac:PartyIdentification/cbc:ID) and normalize-space(cac:Party/cac:PartyIdentification/cbc:ID/@schemeID) = '700') and not(
        starts-with(normalize-space(cac:Party/cac:PartyIdentification/cbc:ID), '7') and
        string-length(normalize-space(cac:Party/cac:PartyIdentification/cbc:ID)) = 10 and
        not(translate((cac:Party/cac:PartyIdentification/cbc:ID), '0123456789', '')))">
            <svrl:failed-assert xmlns:svrl="http://purl.oclc.org/dsdl/svrl"
                                location="{schxslt:location(.)}"
                                ...
                                id="BR-KSA-F-09">
               <xsl:attribute name="test">
                  (exists(cac:Party/cac:PartyIdentification/cbc:ID) and normalize-space(cac:Party/cac:PartyIdentification/cbc:ID/@schemeID) = '700') and not(
                  starts-with(normalize-space(cac:Party/cac:PartyIdentification/cbc:ID), '7') and
                  string-length(normalize-space(cac:Party/cac:PartyIdentification/cbc:ID)) = 10 and
                  not(translate(normalize-space(cac:Party/cac:PartyIdentification/cbc:ID), '0123456789', '')))
               </xsl:attribute>
               <svrl:text>[BR-KSA-F-09] - The value provided in Other Seller ID (BT-29) or Other Buyer ID (BT-46) for the scheme ID '700' appears to be incorrect. Please recheck the value to avoid non-compliance.</svrl:text>
               <svrl:message-code>BR-KSA-F-09</svrl:message-code>
               <svrl:message-category>KSA - custom rules</svrl:message-category>
            </svrl:failed-assert>
         </xsl:if>
         <!-- start new role sprint 14-->
         <!-- Determine context-specific labels for messages -->
         <xsl:variable name="is-supplier" select="self::cac:AccountingSupplierParty"/>
         <xsl:variable name="bt-label"     select="if ($is-supplier) then 'BT-29-1' else 'BT-46-1'"/>
         <xsl:variable name="party-label"  select="if ($is-supplier) then 'Seller'  else 'Buyer'"/>
         <!-- Allowed schemeIDs per context (keep uppercase literals) TIN NAT IQA PAS CRN MOM MLS 700 SAG GCC OTH -->
         <xsl:variable name="allowed-buyer"
                       select="('TIN','CRN','MOM','MLS','700','SAG','NAT','GCC','IQA','OTH','PAS')"/>
         <xsl:variable name="allowed-seller"
                       select="('CRN','MOM','MLS','700','SAG','OTH')"/>
         <xsl:variable name="allowed-here"
                       select="if ($is-supplier) then $allowed-seller else $allowed-buyer"/>
         <!-- The actual @schemeID (raw/trimmed/uppercased) -->
         <xsl:variable name="scheme-raw"
                       select="string(cac:Party/cac:PartyIdentification/cbc:ID/@schemeID)"/>
         <xsl:variable name="scheme-trim"  select="normalize-space($scheme-raw)"/>
         <xsl:variable name="scheme-up"    select="upper-case($scheme-trim)"/>
         <!-- Candidate IDs with an allowed schemeID for this context -->
         <xsl:variable name="ids-allowed"
                       select="cac:Party/cac:PartyIdentification/cbc:ID[
           upper-case(normalize-space(@schemeID)) = $allowed-here
         ]"/>
         <!-- BR-KSA-F-12: allowed scheme but with leading/trailing spaces in @schemeID -->
         <xsl:if test="$scheme-raw
             and $scheme-up = $allowed-here
             and $scheme-raw != $scheme-trim">
            <svrl:failed-assert xmlns:svrl="http://purl.oclc.org/dsdl/svrl"
                                location="{schxslt:location(.)}"
                                flag="warning"
                                id="BR-KSA-F-12">
               <xsl:attribute name="test">
                  $scheme-raw and $scheme-up = $allowed-here and $scheme-raw != $scheme-trim
               </xsl:attribute>
               <svrl:text>
               ...
               <svrl:message-code>BR-KSA-F-12</svrl:message-code>
               <svrl:message-category>KSA - custom rules</svrl:message-category>
            </svrl:failed-assert>
         </xsl:if>
         <!-- BR-KSA-F-13: ONLY check IDs whose schemeID is allowed -->
         <xsl:variable name="bad-pattern-exists"
                       select="exists($ids-allowed[
           matches(normalize-space(.), '^\d+$') and
           (
             matches(normalize-space(.), '(01234567|12345678|23456789)') or
             matches(normalize-space(.), '(76543210|87654321|98765432)') or
             matches(normalize-space(.), '(00000000|11111111|22222222|33333333|44444444|55555555|66666666|77777777|88888888|99999999)')
           )
         ])"/>

         <xsl:if test="$bad-pattern-exists">
            <svrl:failed-assert xmlns:svrl="http://purl.oclc.org/dsdl/svrl"
                                location="{schxslt:location(.)}"
                                flag="warning"
                                id="BR-KSA-F-13">
               <xsl:attribute name="test">$bad-pattern-exists</xsl:attribute>
               <svrl:text>
                  [BR-KSA-F-13] - Please recheck Other Seller/Buyer ID (BT-29 or BT-46) as it contains an invalid value.
               </svrl:text>
               <svrl:message-code>BR-KSA-F-13</svrl:message-code>
               <svrl:message-category>KSA - custom rules</svrl:message-category>
            </svrl:failed-assert>
         </xsl:if>
         <!-- end new role sprint 14-->
      </schxslt:rule>
      <xsl:next-match>
         <xsl:with-param name="schxslt:patterns-matched" as="xs:string*" select="($schxslt:patterns-matched, 'd7e29')"/>
      </xsl:next-match>
   </xsl:template>
   <!-- end new template sprint-13 -->
      ...
      <xsl:choose>
         ...
         <xsl:otherwise>
            <schxslt:rule pattern="d7e29">
               ...
               <xsl:if test="not( string-length(normalize-space( ./@schemeID)) &gt; 2 and contains('TIN NAT IQA PAS CRN MOM MLS 700 SAG GCC OTH', normalize-space(./@schemeID)))">
               ...
               <xsl:if test="normalize-space(@schemeID) = 'TIN' and
                         not(string-length(normalize-space(.)) = 10
                             and not(translate((.), '0123456789', ''))
                             and starts-with(normalize-space(.), '3'))">
                  <svrl:failed-assert xmlns:svrl="http://purl.oclc.org/dsdl/svrl"
                                      location="{schxslt:location(.)}"
                                      flag="warning"
                                      id="BR-KSA-F-07">
                     ...
                     <svrl:text>[BR-KSA-F-07] - The value provided in Other Buyer ID (BT-46) for scheme ID 'TIN' appears to be incorrect. Please recheck the value to avoid non-compliance.
                     </svrl:text>
                     <svrl:message-code>BR-KSA-F-07</svrl:message-code>
                     <svrl:message-category>KSA - custom rules</svrl:message-category>
                  </svrl:failed-assert>
               </xsl:if>

               <xsl:if test="(exists(.) and normalize-space(@schemeID) = 'NAT') and not(
        starts-with(normalize-space(.), '1') and
        string-length(normalize-space(.)) = 10 and
        not(translate((.), '0123456789', '')))">
                  <svrl:failed-assert xmlns:svrl="http://purl.oclc.org/dsdl/svrl"
                                      location="{schxslt:location(.)}"
                                      flag="warning"
                                      id="BR-KSA-F-10">
                     <xsl:attribute name="test">
                        (exists(cac:Party/cac:PartyIdentification/cbc:ID) and normalize-space(cac:Party/cac:PartyIdentification/cbc:ID/@schemeID) = 'NAT') and not(
                        starts-with(normalize-space(cac:Party/cac:PartyIdentification/cbc:ID), '1') and
                        string-length(normalize-space(cac:Party/cac:PartyIdentification/cbc:ID)) = 10 and
                        not(translate(normalize-space(cac:Party/cac:PartyIdentification/cbc:ID), '0123456789', '')))
                     </xsl:attribute>
                     <svrl:text>[BR-KSA-F-10] - The value provided in Other Buyer ID (BT-46) for the scheme ID 'NAT' appears to be incorrect. Please recheck the value to avoid non-compliance.</svrl:text>
                     <svrl:message-code>BR-KSA-F-10</svrl:message-code>
                     <svrl:message-category>KSA - custom rules</svrl:message-category>
                  </svrl:failed-assert>
               </xsl:if>

               <xsl:if test="normalize-space(@schemeID) = 'IQA' and
                         not(string-length(normalize-space(.)) = 10
                             and not(translate((.), '0123456789', ''))
                             and starts-with(normalize-space(.), '2'))">
                  <svrl:failed-assert xmlns:svrl="http://purl.oclc.org/dsdl/svrl"
                                      location="{schxslt:location(.)}"
                                      flag="warning"
                                      id="BR-KSA-F-11">
                     ...
                     <svrl:text>[BR-KSA-F-11] - The value provided in Other Buyer ID (BT-46) for the scheme ID 'IQA' appears to be incorrect. Please recheck the value to avoid non-compliance. </svrl:text>
                     <svrl:message-code>BR-KSA-F-11</svrl:message-code>
                     <svrl:message-category>KSA - custom rules</svrl:message-category>
                  </svrl:failed-assert>
               </xsl:if>
            </schxslt:rule>
               ...
               <xsl:if test="boolean(//*[matches(@name, '01\d{5}')])">
                  <xsl:if test="not (string-length(//cac:AccountingCustomerParty/cac:Party/cac:PostalAddress/cac:Country/cbc:IdentificationCode) &gt; 0 and string-length(//cac:AccountingCustomerParty/cac:Party/cac:PostalAddress/cbc:StreetName) &gt; 0   and string-length(//cac:AccountingCustomerParty/cac:Party/cac:PostalAddress/cbc:CityName) &gt; 0 and exists(//cac:AccountingCustomerParty/cac:Party/cac:PostalAddress/cbc:StreetName) and exists(//cac:AccountingCustomerParty/cac:Party/cac:PostalAddress/cbc:CityName)  and exists(//cac:AccountingCustomerParty/cac:Party/cac:PostalAddress/cac:Country/cbc:IdentificationCode))">
                     <svrl:failed-assert xmlns:svrl="http://purl.oclc.org/dsdl/svrl" location="{schxslt:location(.)}" flag="warning" id="BR-KSA-10">
                        <xsl:attribute name="test">  exists(//cac:AccountingCustomerParty/cac:Party/cac:PostalAddress/cbc:StreetName) and   exists(//cac:AccountingCustomerParty/cac:Party/cac:PostalAddress/cbc:CityName) and   exists(//cac:AccountingCustomerParty/cac:Party/cac:PostalAddress/cbc:PostalZone) and exists(//cac:AccountingCustomerParty/cac:Party/cac:PostalAddress/cbc:CitySubdivisionName) and  exists(//cac:AccountingCustomerParty/cac:Party/cac:PostalAddress/cac:Country/cbc:IdentificationCode)</xsl:attribute>
                        <svrl:text>[BR-KSA-10]-Where the Buyer Country Code (BT-55) is other than 'SA' Buyer address must contain a street (BT-50), city (BT-52), country code (BT-55). This rule does not apply on the simplified tax invoices and associated credit notes and debit notes (KSA-2, position 1 and 2 = 02).</svrl:text>
      ...
      <xsl:choose>
         ...
         <xsl:otherwise>
            <schxslt:rule pattern="d7e276">
               ...
               <xsl:if test="not(                 (substring(./@name, 3,1) = '1' or substring(./@name, 3,1) = '0') and                  (substring(./@name, 4,1) = '1' or substring(./@name, 4,1) = '0') and                 (substring(./@name, 5,1) = '1' or substring(./@name, 5,1) = '0') and                 (substring(./@name, 6,1) = '1' or substring(./@name, 6,1) = '0') and                  (substring(./@name, 7,1) = '0'))">
               <svrl:failed-assert xmlns:svrl="http://purl.oclc.org/dsdl/svrl" location="{schxslt:location(.)}" flag="error" id="BR-KSA-31">
                     ...
                     <svrl:text>
                        [BR-KSA-31]- For Simplified Tax Invoices and associated credit notes and debit notes (KSA-2, position 1 and 2 = 02), transaction types "self-billing" (KSA-2, position 7 = True) is not allowed.</svrl:text>
